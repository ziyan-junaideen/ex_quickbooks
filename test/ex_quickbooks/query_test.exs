defmodule ExQuickbooks.QueryTest do
  ...
  test "run/3 appends pagination clauses and returns QueryResponse" do
    bypass = Bypass.open()
    client = client()
    ...
    assert {:ok, %{"Customer" => [%{"Id" => "123"}], "startPosition" => 10, "maxResults" => 25}} =
  ...
  test "run/3 preserves caller-supplied pagination clauses" do
    ...
    assert query_response["CompanyInfo"] == [%{"CompanyName" => "Acme LLC"}]
  end

  test "top_level_collection/1 extracts the primary collection from QueryResponse" do
